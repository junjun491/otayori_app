...
export default function InvalidInvitationPage() {
  ...
  return (
    <Container maxWidth="sm">
      <Box mt={8}>
        <Typography variant="h4" gutterBottom>招待を確認できません</Typography>
        <Alert severity="error" sx={{ mb: 3 }}>{messageFor(reason)}</Alert>
        <Button variant="outlined" onClick={() => router.replace('/login')}>ログインへ戻る</Button>
      </Box>
    </Container>
  );
}
